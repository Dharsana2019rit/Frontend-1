      ...
      try {
        ...
        const bookingsWithCustomers = await Promise.all(bookingsData.map(async (booking) => {
          ...
          const formattedBooking = {
            ...booking,
            bookingDate: new Date(booking.bookingDateTime).toLocaleDateString(),
            bookingTime: new Date(booking.bookingDateTime).toLocaleTimeString(),
          ...
          {bookings.length === 0 ? (
            ...
            bookings.map((booking, index) => (
              <div className="table-card-admin" key={index}>
                ...
                <p><strong>Booking Time:</strong> {booking.bookingTime}</p> {/* Display booking time */}
